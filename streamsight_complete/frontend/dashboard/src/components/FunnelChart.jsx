...
// ── STAGES — unchanged ──────────────────────────────────────────────────────
const STAGES = [
  { key: "page_view",   label: "Page Views",  color: "#1D9E75", iconBg: "#E1F5EE", icon: "👁"  },
  { key: "add_to_cart", label: "Add to Cart", color: "#378ADD", iconBg: "#E6F1FB", icon: "🛒" },
  { key: "checkout",    label: "Checkout",    color: "#7F77DD", iconBg: "#EEEDFE", icon: "💳" },
  { key: "purchase",    label: "Purchase",    color: "#1D9E75", iconBg: "#E1F5EE", icon: "✅" },
];
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 22 }}>
        <div>
          ...
          <div style={{ color: "#475569", fontSize: 12, marginTop: 3 }}>Real-time user journey</div>
        </div>
        <div style={{
          ...
          <span style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", color: "#1D9E75", fontWeight: 600 }}>LIVE</span>
        </div>
      </div>
      ...
      {isEmpty ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12 }}>
          ...
          <span style={{ color: "#475569", fontSize: 13 }}>Waiting for events...</span>
        </div>
      ) : (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 18 }}>
          {STAGES.map((stage, i) => {
            ...
            const isHovered = hovered === i;
            const dropoff   = i < STAGES.length - 1 && values[i] > 0
              ? ((1 - values[i + 1] / values[i]) * 100).toFixed(1) : null;
            const cvr = i > 0 && values[i - 1] > 0
              ? ((val / values[i - 1]) * 100).toFixed(1) : null;
            ...
            return (
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <motion.div
                      ...
                      style={{
                        ...
                        background: isHovered ? stage.iconBg : "rgba(255,255,255,0.04)",
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {dropoff && (
                      <span style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", color: "#E24B4A", opacity: 0.8 }}>
                        ↓ {dropoff}% drop
                      </span>
                    )}
                    <motion.span
                      animate={{ color: isHovered ? stage.color : "#E2E8F0" }}
                      style={{ fontSize: 18, fontWeight: 800, fontVariantNumeric: "tabular-nums", fontFamily: "'DM Sans',sans-serif" }}
                    >
                      {val.toLocaleString()}
                    </motion.span>
                  </div>
                ...
                <div style={{
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      transition={isHovered ? { duration: 0.65, ease: "easeInOut" } : {}}
                      style={{
                ...
                <AnimatePresence>
                  {cvr && isHovered && (
                    ...
                    >
                      <span style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", color: `${stage.color}90` }}>
                  ...
                  {cvr && !isHovered && (
                    <motion.div
                      ...
                      style={{ marginTop: 5, textAlign: "right" }}
                    >
                      <span style={{ fontSize: 10, fontFamily: "'DM Mono',monospace", color: `${stage.color}55` }}>
                        {cvr}% conversion from previous
                      </span>
                    </motion.div>
                  )}
          ...
          {values[0] > 0 && (
            ...
            >
              <span style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", color: "#475569", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                Overall CVR
              </span>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ height: 4, width: 60, borderRadius: 2, background: "rgba(29,158,117,0.15)", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${((values[3] / values[0]) * 100).toFixed(1)}%`, background: "#1D9E75", borderRadius: 2 }} />
                </div>
                <span style={{ fontSize: 18, fontWeight: 800, color: "#1D9E75", fontFamily: "'DM Sans',sans-serif", fontVariantNumeric: "tabular-nums" }}>
                  {((values[3] / values[0]) * 100).toFixed(2)}%
                </span>
              </div>
            </motion.div>
